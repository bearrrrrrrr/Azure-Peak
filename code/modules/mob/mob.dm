 ...
 */
/atom/proc/audible_message(message, deaf_message, hearing_distance = DEFAULT_MESSAGE_RANGE, self_message, runechat_message = null, log_seen = NONE, log_seen_msg = null, is_emote = TRUE)
	...
	for(var/mob/M in hearers)
		...
		if(runechat_message && M.can_see_runechat(src) && M.can_hear())
			M.create_chat_message(src, raw_message = runechat_message, spans = (is_emote ? list("emote") : null))
	if(log_seen)
		log_seen(src, null, hearers, (log_seen_msg ? log_seen_msg : message), log_seen)
 ...
 * * hearing_distance (optional) is the range, how many tiles away the message can be heard.
 */
/mob/audible_message(message, deaf_message, hearing_distance = DEFAULT_MESSAGE_RANGE, self_message, runechat_message = null, log_seen = NONE, log_seen_msg = null, is_emote = TRUE)
	. = ..()
	if(self_message)
