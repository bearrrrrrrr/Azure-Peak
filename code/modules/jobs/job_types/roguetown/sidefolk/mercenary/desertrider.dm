...
/datum/status_effect/buff/zeybek_momentum/proc/grant_milestone_boost(milestone)
	...
	queue_milestone_resource_restore()
...
/datum/status_effect/buff/zeybek_momentum/proc/queue_milestone_resource_restore()
	if(!owner)
		return
	var/stamina_restore = owner.max_stamina * 0.25
	var/fatigue_restore = owner.max_energy * 0.05
	addtimer(CALLBACK(src, PROC_REF(apply_milestone_resource_restore), stamina_restore, fatigue_restore), 0)

/datum/status_effect/buff/zeybek_momentum/proc/apply_milestone_resource_restore(stamina_restore, fatigue_restore)
	if(!owner)
		return
